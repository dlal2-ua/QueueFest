...
export function BarOffersScreen() {
  ...
  const [barNombre, setBarNombre] = useState<string>(`Barra #${id}`);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error('Error cargando ofertas de barra:', err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{barNombre}{festival?.nombre ? ` · ${festival.nombre}` : ''}</p>
          </div>
        </div>
      </div>
