      ...
      <div className="w-full max-w-xs">
        <div className="flex items-center gap-3 rounded-2xl px-5 py-4 border"
          ...
          <select
            ...
            style={{ color: selectedId ? '#FFFFFF' : '#888888' }}
          >
            <option value="" disabled style={{ color: '#333' }}>Elige un festival…</option>
            {festivales.map(f => (
              <option key={f.id} value={f.id} style={{ color: '#111', backgroundColor: '#1A1A1A' }}>
                {f.nombre}{!f.activo ? ' (inactivo)' : ''}
              </option>
            ))}
          </select>
          <ArrowRight className="w-4 h-4 flex-shrink-0" style={{ color: '#FF6B35' }} />
        </div>

        {/* Enter button — aparece con fade cuando hay selección */}
        <div className={`mt-4 transition-all duration-300 ${selectedId ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 pointer-events-none'}`}>
          ...
          >
            {entering ? 'Cargando…' : 'Entrar al festival →'}
          </button>
        </div>
      </div>
    </div>
